...
const page = () => {
  ...
  const handelGetWeight = async () => {
    try {
      const { data } = await api.get(`/api/v1/members/body?dateTime=2024-01-08`)
      if (data) {
        setTodayWeight({
          ...todayWeight,
          ...data.data
  ...
  const handleWeightSave = async () => {
    const response = await api.post(`/api/v1/members/body/weight`, {
